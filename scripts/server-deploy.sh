...
SESSION_MAX_AGE_HOURS=${SESSION_MAX_AGE_HOURS:-720}
DINGTALK_WEBHOOK=${DINGTALK_WEBHOOK:-}
DINGTALK_SECRET=${DINGTALK_SECRET:-}
RETURN_DINGTALK_WEBHOOK=${RETURN_DINGTALK_WEBHOOK:-}
RETURN_DINGTALK_SECRET=${RETURN_DINGTALK_SECRET:-}
REPAIR_DINGTALK_WEBHOOK=${REPAIR_DINGTALK_WEBHOOK:-}
REPAIR_DINGTALK_SECRET=${REPAIR_DINGTALK_SECRET:-}
...
npm ci
npm run db:init
if [ "${PREBUILT_DIST:-false}" = "true" ]; then
  test -f server/dist/server.js
  test -f server/dist/public/index.html
else
  npm run build
fi
